...
class BurpExtender(IBurpExtender,IMessageEditorTabFactory,IProxyListener,IHttpListener,ITab,IMessageEditorController):
	...
	def getUiComponent(self):
		...
		self.dontRepeatRequests=JCheckBox("Don't Repeat Requests");
		self.scanPostRequests=JCheckBox("Scan POST Requests");
		...
		self.scanPostRequests.setAlignmentX(Component.CENTER_ALIGNMENT)
		...
		configPanel.add(Box.createRigidArea(Dimension(0, 20)))
  		...
  		configPanel.add(self.scanPostRequests)
		configPanel.add(Box.createRigidArea(Dimension(0, 20)))
	...
	def xssChecker(self,messageInfo):
		...
		for param in (params):
			...
			if not (paramType==param.PARAM_URL or ((paramType==param.PARAM_BODY or paramType==param.PARAM_JSON) and self.scanPostRequests.isSelected())):
				continue
			newReq=messageInfo.getRequest()
			payloads=[["xuicodexua\\",["xuicodexua\\'","xuicodexua\\\""]],["xuicodexub\"",["xuicodexub\"\"","xuicodexub\\\"\\\""]],["xuicodexuc'",["xuicodexuc''","xuicodexuc\\'\\'"]],["xuicodexud\\\"",["xuicodexud\\\\\""]]]
	...
	def processHttpMessage(self, toolFlag, messageIsRequest, messageInfo):
		if messageIsRequest and self.isRunning:
			...
			if (method=='GET' or (method=='POST' and self.scanPostRequests.isSelected())) and len(params) and toolFlag == self._callbacks.TOOL_PROXY and ((self._callbacks.isInScope(url) and self.inScopeCheckBox.isSelected()) or not self.inScopeCheckBox.isSelected()):
				start_new_thread(self.xssChecker,(messageInfo,))
			# elif method=='POST':
			# 	for param in params:
			# 		self._stdout.println(param.getName())


		# elif messageIsResponse and self.checkForHunterx.isSelected():
			#EE
