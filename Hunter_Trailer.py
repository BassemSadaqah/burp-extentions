...
class BurpExtender(IBurpExtender,IMessageEditorTabFactory,IProxyListener,IHttpListener,ITab,IMessageEditorController):
	...
	urls_log=[]
	isRunning=True
	payload_headers=[['Trailer','huicodehui']]
	...
	def registerExtenderCallbacks(self, callbacks):
		...
		callbacks.setExtensionName("Hunter Trailer") 
		# callbacks.registerProxyListener(self)
		callbacks.registerHttpListener(self)
		return

	def getTabCaption(self):
		return "Hunter Trailer"
	...
	def startOrStop(self, event):
		if self.startButton.getText() == "Hunter Trailer is Off":
			self.startButton.setText("Hunter Trailer is On")
			self.startButton.setSelected(True)
			self.isRunning = True
		else:
			self.startButton.setText("Hunter Trailer is Off")
	...
	def getUiComponent(self):
		...
		self.startButton=JToggleButton("Hunter Trailer is On",actionPerformed=self.startOrStop)
		self.startButton.setSelected(True)
	...
	def makeReqWithPayloads(self,request_response,req_headers,payload_headers):
		...
		req=self._helpers.analyzeRequest(request_response.getRequest())
		body_offset=req.getBodyOffset()
		newReq=self._helpers.buildHttpMessage(filtered_req_headers+filtered_header_parsed,request_response.getRequest()[body_offset:])
	...
	def processHttpMessage(self, toolFlag, messageIsRequest, request_response):
		...
		if self.isRunning and (method=='GET' or method=='POST') and toolFlag == self._callbacks.TOOL_PROXY and ((self._callbacks.isInScope(url) and self.inScopeCheckBox.isSelected()) or not self.inScopeCheckBox.isSelected()):
			if  messageIsRequest==False :
				...
				request_response.setRequest(self._helpers.buildHttpMessage(new_headers,req_bytes[body_offset:]))
